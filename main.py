...
def fetch_and_update_parcels():
    ...
    # ✅ Process all parcels, filling in missing values with defaults
    processed_parcels = []
    for parcel in parcels_list:
        ...
        if not isinstance(last_scanned, dict):  # If it's None or another type, replace it with an empty dict
            last_scanned = {}
        ...
        processed_parcels.append({
            ...
            "address": {
                "name": parcel.get("address", {}).get("name", "Unknown Location"),
                "address1": parcel.get("address", {}).get("address1", ""),
                "address2": parcel.get("address", {}).get("address2", ""),
                "city": parcel.get("address", {}).get("city", "Unknown City"),
                "state": parcel.get("address", {}).get("state", "Unknown State"),
                "zip": parcel.get("address", {}).get("zip", "00000")
            },
